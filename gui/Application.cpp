...
bool Application::OnInit() {
    ...
    if (!SettingsLoad()) {
        if (ShowSettings() != wxID_OK) {
            wxMessageBox("Des paramètres valides sont requis.", "Erreur", wxOK, nullptr);
            return false;
        } else {
            if (!SettingsSave()) {
                wxMessageBox("Erreur lors de la sauvegarde.", "Erreur", wxOK, nullptr);
                return false;
            }
        }
    }
    ...
    for (const auto &game : games) {
        ...
        if (!PluginInstaller::MaybeUpdate(game.first, game.second, error)) {
            wxMessageBox(error, "Erreur", wxOK, nullptr);
            if (ShowSettings() != wxID_OK) {
                wxMessageBox("Le plugin doit être installé.", "Erreur", wxOK, nullptr);
                return false;
            }
        }
    }
...
int Application::ShowSettings() {
    ...
    if (ret == wxID_OK) {
        if (!SettingsSave()) {
            wxMessageBox("Erreur lors de la sauvegarde.", "Erreur", wxOK, m_window);
        }
    }
    delete dialog;
