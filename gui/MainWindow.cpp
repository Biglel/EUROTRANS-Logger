...
void MainWindow::on_sender_update(wxThreadEvent &event) {
    ...
    if (event.GetId() == wxID_ABORT) {
        ...
        notificationMessage.SetMessage("Votre livraison à bien été envoyé !");
    }
...
bool MainWindow::can_close() {
    ...
    if (m_sender->hasPending()) {
        wxString message("Il reste des livraisons à envoyer. Êtes vous sûr de vouloir quitter ?");
        ret = (wxMessageBox(message, "Attention", wxYES_NO, nullptr) == wxYES);
    } else {
        ret = true;
    }
...
bool MainWindow::start() {
    ...
    if (!m_sender->start()) {
        wxMessageBox("Erreur durant la création du LockGuard (Erreur 100)", "Erreur", wxOK, this);
        return false;
    }
