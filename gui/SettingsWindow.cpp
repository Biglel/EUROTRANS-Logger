...
void SettingsWindow::on_click_ok(wxCommandEvent &/*event*/) {
    if (!m_checkBoxETS2->IsChecked() && !m_checkBoxATS->IsChecked()) {
        wxMessageBox("Le plugin doit être installé sur au moins un jeux.", "Erreur", wxOK, this);
        return;
    }

    if (m_checkBoxETS2->IsChecked()) {
        if (m_dirCtrlETS2Path->GetPath().empty()) {
            wxMessageBox("Le chemin de ETS2 doit être valide", "Erreur", wxOK, this);
            return;
        } else {
            wxString error;
            if (!PluginInstaller::MaybeUpdate("ETS2", m_dirCtrlETS2Path->GetPath(), error)) {
                wxMessageBox(error, "Erreur", wxOK, this);
                return;
            }
    ...
    if (m_checkBoxATS->IsChecked()) {
        if (m_dirCtrlATSPath->GetPath().empty()) {
            wxMessageBox("Le chemin de ATS doit être valide", "Erreur", wxOK, this);
            return;
        } else {
            wxString error;
    ...
    if (m_textCtrlURL->GetValue().empty()) {
        wxMessageBox("L'URL ne peut pas être vide", "Erreur", wxOK, this);
        return;
    } else {
    ...
    if (m_textCtrlToken->GetValue().empty()) {
        wxMessageBox("Le Token ne peut pas être vide", "Erreur", wxOK, this);
        return;
    } else {
