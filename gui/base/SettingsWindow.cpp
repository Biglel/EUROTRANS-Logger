...
SettingsWindow::SettingsWindow( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxDialog( parent, id, title, pos, size, style )
{
	...
	sbSizerGame = new wxStaticBoxSizer( new wxStaticBox( this, wxID_ANY, wxT("Jeux") ), wxVERTICAL );

	m_checkBoxETS2 = new wxCheckBox( sbSizerGame->GetStaticBox(), wxID_ANY, wxT("Installation du plugin pour ETS2"), wxDefaultPosition, wxDefaultSize, 0 );
	sbSizerGame->Add( m_checkBoxETS2, 0, wxALL, 5 );

	wxStaticText* m_staticTextETS2Path;
	m_staticTextETS2Path = new wxStaticText( sbSizerGame->GetStaticBox(), wxID_ANY, wxT("Chemin d'accès ETS2"), wxDefaultPosition, wxDefaultSize, 0 );
	m_staticTextETS2Path->Wrap( -1 );
	sbSizerGame->Add( m_staticTextETS2Path, 0, wxALL, 5 );

	m_dirCtrlETS2Path = new wxDirPickerCtrl( sbSizerGame->GetStaticBox(), wxID_ANY, wxEmptyString, wxT("Selectionnez le dossier de ETS2"), wxDefaultPosition, wxDefaultSize, wxDIRP_DEFAULT_STYLE );
	sbSizerGame->Add( m_dirCtrlETS2Path, 0, wxALL|wxEXPAND, 5 );

	m_checkBoxATS = new wxCheckBox( sbSizerGame->GetStaticBox(), wxID_ANY, wxT("Installation du plugin pour ATS"), wxDefaultPosition, wxDefaultSize, 0 );
	sbSizerGame->Add( m_checkBoxATS, 0, wxALL, 5 );

	wxStaticText* m_staticTextATSPath;
	m_staticTextATSPath = new wxStaticText( sbSizerGame->GetStaticBox(), wxID_ANY, wxT("Chemin d'accès ATS"), wxDefaultPosition, wxDefaultSize, 0 );
	m_staticTextATSPath->Wrap( -1 );
	sbSizerGame->Add( m_staticTextATSPath, 0, wxALL, 5 );

	m_dirCtrlATSPath = new wxDirPickerCtrl( sbSizerGame->GetStaticBox(), wxID_ANY, wxEmptyString, wxT("Selectionnez le dossier de ATS"), wxDefaultPosition, wxDefaultSize, wxDIRP_DEFAULT_STYLE );
